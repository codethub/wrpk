...
const uploadFileToIPFS = async (file: File): Promise<string> => {
    ...
    const uploadImg = await fetch("/api/upload-ipfs", {
        method: "POST",
        body: formData,
    });
    ...
    return `ipfs://${uploadImgData.Hash}`;
};


export default function MintUp() {
  ...
  const isUploadReady = useMemo(() => !!uploadedFile, [uploadedFile]);
  ...
  const handleGenerate = async () => {
    ...
    if (!uploadedFile) {
        setUiError("Please upload an image before generating.");
        return;
    }
    ...
    try {
        setUiError("Uploading image to IPFS...");
        imageIpfsUrl = await uploadFileToIPFS(uploadedFile);
        
        const response = await fetch('/api/warpunk-up', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                userAddress: address,
                uploadedImageUrl: imageIpfsUrl,
                fid: profile.fid,
    ...
    <main className="flex-1 w-full flex flex-col items-center p-4 md:p-10 py-10 pt-20 pb-20">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-slate-400 text-sm md:text-base max-w-sm mx-auto">
          Merge your Custom Image through the Warpunk AI & Mint to NFT.
        </p>
      </div>
      ...
      <div className="relative flex flex-col items-center p-6 md:p-8 bg-slate-900/70 backdrop-blur-md border border-cyan-500/30 rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden">
        ...
        {!isConnected ? (
          ...
          <div className="w-full text-center">
            ...
            {profile && (
              <div className="mb-8 p-4 bg-slate-800/50 border border-slate-700 rounded-lg flex flex-col items-center justify-center gap-4 shadow-inner">
                <p className="text-xl font-extrabold text-white tracking-widest uppercase">
                  @{profile.username}
                </p>
                <p className="text-sm text-slate-400">Use your own image for Warpunk fusion.</p>
              </div>
            )}
            
            {!hasMinted && (
                <div className="mb-6 p-4 bg-slate-800/50 border border-slate-700 rounded-lg">
                    <label className="text-sm font-semibold text-white mb-2 block text-left">
                        Image for Fusion (Image 2)
                    </label>
                    <input 
                        type="file" 
                    ...
                    {uploadedImagePreview ? (
                        <div className="mt-4 p-2 bg-slate-900 rounded-lg flex items-center gap-3">
                            <img src={uploadedImagePreview} alt="Uploaded Preview" className="w-16 h-16 rounded-lg object-cover border border-cyan-500/50" />
                            <p className="text-sm text-green-400">Image ready for fusion!</p>
                        </div>
                    ) : (
                        <p className="mt-2 text-xs text-slate-500 text-left flex items-center gap-2">
                             <Camera size={14} /> Upload a PNG/JPG/WEBP image.
                        </p>
                    )}
                </div>
            )}
